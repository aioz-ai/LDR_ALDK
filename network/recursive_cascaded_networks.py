...
class RecursiveCascadedNetworks(Network):
    ...
    def __init__(self, name, framework,
                 ...
                 aldk=False, supervision=False, no_init_stem=False,
                 flow_multiplier=1.0, beta=0., lamb=10., gamma=0.5,
                 **kwargs):
        super().__init__(name)
        ...
        self.aldk = aldk
        self.no_init_stem = no_init_stem
    ...
    def build(self, img1, img2, seg1, seg2, pt1, pt2, pretrained_flow, gamma):
        ...
        # Cascade Affine
        affine_params, one_cascade_params = 0, 0
        if not self.no_init_stem:
            stem_result = self.stems[0][0](img1, img2)
            affine_params = self.get_model_no_params(self.stems[0][0])
            ...
            # Cascade Deformation Only
            stem_result = None
            for stem, params in self.stems:
                if stem_result != None:
                    flows.append(stem_result['agg_flow'])
                if self.warp_gradient:
        ...
        if self.supervision or self.aldk:
        ...
        if self.no_init_stem:
            n_cascades = len(self.stems)
        else:
            n_cascades = len(self.stems) - 1
        """
        print('number of Cascade: ', n_cascades)
        print('Total params: ', total_parameters)
        print('Affine Params: ', affine_params)
        print('One cascade part params: ', one_cascade_params)
        assert total_parameters == n_cascades*one_cascade_params + affine_params
        """
